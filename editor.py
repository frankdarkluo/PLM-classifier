...
class RobertaEditor(nn.Module):
    ...
    def generate(self, input_texts, max_len):
        ...
        for input_text in input_texts:
            for mask_word in mask_words:
                cand_sent = input_text.replace("<mask>", mask_word).lower()
    ...
    def get_contextual_word_embeddings(self, input_texts):
        #inputs1 = {k: v.to(device) for k, v in self.tokenizer(input_texts, padding=True, return_tensors="pt").items()}
        inputs={}
